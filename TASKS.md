...
- Раздел `Товары` расширен до управления карточками товара после локальной проверки.
...
- Live создание нового скрытого товара через Telegram еще не выполнялось.
- Excel/CSV import метаданных товара остается следующим большим этапом после live-проверки CRUD.
